...
void DFS(int k, int mygraph) {
	...
	if (mygraph == 0){
		...
		for (int j = GfirstOut[k]; j < GfirstOut[k+1]; j++) {
			...
			v = Gout[j];
			e = positionOut[j];
		    if (!label2pre[v] && edge_state[e] == unused) {
				...
				root[v] = f_root;
				DFS(v, mygraph);
				parent[current_Tree][v] = k;
				parent_edge_id[current_Tree][v] = e;
				edge_state[e] = current_Tree;
				depths[current_Tree][v] = mydepths;
				mydepths--;		
			}
		}
	  ...
	  int z;
		...
		if (mygraph == 0){
			...
			to = temp;				
	   													...
	   													//initalizing variables and data structures
	    current_Tree = -1;
		 init();											// we are in tree 1
	  
	 // compute_first_tree();											//computing the first tree with dfs													      
