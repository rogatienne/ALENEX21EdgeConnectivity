/**********************************************************
   ...
   -Executes both directions (s-out and s-in) in parallel
   -Initializes the new forest by DFS trees
***********************************************************/
...
int depth_value = 0; //global variable for dfs
int mydepth = 0;
...
void DFS(int k, int mygraph) {

    int v;
	int e;
	/* dfs routine in order to construct the first tree T1 */
	...
	if (mygraph == 0){
		...
		for (int j = GfirstOut[k]; j < GfirstOut[k+1]; j++) {
			...
			v = Gout[j];
			e = positionOut[j];
		    if (!label2pre[v] && myedge_state[e] == unused) {
				...
				root[v] = f_root;
				DFS(v, mygraph);
				myparents[current_Tree][v] = k;
				myparent_edge_id[current_Tree][v] = e;
				myedge_state[e] = current_Tree;
				mydepths[current_Tree][v] = depth_value;
				depth_value--;		
			}
		}
		...
		for (int j = GfirstIn[k]; j < GfirstIn[k+1]; j++) {
...
void init_forest(int mygraph){

/* This function inits the forest before we go to gabow procedure*/
